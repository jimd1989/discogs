module Datasource.Models.Arguments (Args(..), parseArgs, Args'(..)) where
...
data Args' = Args' {
  ...
  genre' ∷ T.Text,
  url' ∷ T.Text, 
  files' ∷ NonEmpty T.Text
}

makeFiles' ∷ [String] → Either String (NonEmpty T.Text)
makeFiles' = note "audio files not provided" . nonEmpty . (T.pack . wrap) ◁ drop 2
